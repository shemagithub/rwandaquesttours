...
describe('handleServerError', () => {
  ...
  it('uses response data.error when title is missing', () => {
    const error = new AxiosError('Request failed')
    error.response = {
      status: 500,
    ...
    expect(toastError).toHaveBeenCalledWith('Duplicate entry')
  })

  it('maps Axios 401 with empty body to a clear auth hint', () => {
    ...
    expect(toastError).toHaveBeenCalledWith(
      'Unauthorized (invalid email/password or expired session).',
    )
  })
